...
ifeq ($(sysname), $(cygwinname))
	javac --module-path "C:/Program Files/Java/javafx/lib" --add-modules ALL-MODULE-PATH -cp ".;" -parameters -d build `find src/ -name \*.java`
else
	javac --module-path "C:/Program Files/Java/javafx/lib" --add-modules ALL-MODULE-PATH -cp ".:" -parameters -d build `find src/ -name \*.java`
endif
...
.PHONY: run
run:
	java --module-path "C:/Program Files/Java/javafx/lib" --add-modules ALL-MODULE-PATH -cp JJSP.jar jjsp.engine.Launcher 
